...
import sys
import config
from models.drau_glove import drau_glove
...
def train():
    ...
    for iter_idx, (data, word_length, feature, answer, glove, epoch) in enumerate(train_Loader):
        ...
        if iter_idx % opt.PRINT_INTERVAL == 0 and iter_idx != 0:
            ...
            writer.add_scalar('drau_glove/train_loss', c_mean_loss, iter_idx)
            writer.add_scalar('drau_glove/lr', optimizer.param_groups[0]['lr'], iter_idx)
        ...
        if iter_idx % opt.CHECKPOINT_INTERVAL == 0 and iter_idx != 0:
            if not os.path.exists('./data'):
                os.makedirs('./data')
            save_path = './data/drau_glove_iter_' + str(iter_idx) + '.pth'
        ...
        if iter_idx % opt.VAL_INTERVAL == 0 and iter_idx != 0:
            ...
            writer.add_scalar('drau_glove/val_loss', test_loss, iter_idx)
            writer.add_scalar('drau_glove/accuracy', acc_overall, iter_idx)
            ...
            drawgraph(results, folder, opt.IMAGE_CHANNEL, opt.FUSION_OUT_DIM, prefix='drau_glove')
...
folder = 'drau_glove_%s'%opt.TRAIN_DATA_SPLITS
...
if opt.RESUME:
    ...
    for name, param in model.named_parameters():
        ...
        elif 'prelu' in name:  # Prelu can't be init by xavier
            init.constant_(param, 0.0)
        elif 'weight_ih_l' in name or 'weight_hh_l' in name:
